<template>
  <div class="Pokemon">
    ...
    <div class="details">
      ...
      <h1>{{ Pokemon.name }}</h1>
      <h4>Trainer<strong>{{ Pokemon.trainer }}</strong></h4>
...
<script>
import { GET_POKEMON, ALL_POKEMONS, DELETE_POKEMON } from "../constants/";
export default {
  data() {
    return {
      detailID: null,
      ...
      this.$apollo
        .mutate({
          ...
          update: (store, { data: { deletePokemon } }) => {
            const { allPokemons } = store.readQuery({ query: ALL_POKEMONS });
            store.writeQuery({
              query: ALL_POKEMONS,
              data: {
                allPokemons: allPokemons.filter(n => n.id !== deletePokemon.id)
              }
            });
          }
        })
